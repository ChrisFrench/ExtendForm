...
class plgSystemExtendForm extends JPlugin {
	...
	function onContentPrepareForm($form, $data) {
		...
		$query->where($db->quoteName('enabled') . ' = 1 AND ' .$db->quoteName('formname') .  " = '" . $form -> getName() . "'" );
        ...
        if (!in_array($form->getName(), array($result->formname))) {
            return true;
        }
		...
		if(empty($result->path)) {
			...
			$form -> loadFile($result->xmlfile, false);
		}
	}
}
?>